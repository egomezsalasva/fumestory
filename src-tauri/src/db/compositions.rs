...
use super::{ensure_can_create_composition, ensure_can_create_mod, map_sqlite_error, open_db};
...
fn assert_dilutions_exist(conn: &rusqlite::Connection, dilution_ids: &[i64]) -> Result<(), String> {
...
#[tauri::command]
pub fn db_list_compositions(app: AppHandle, status: String) -> Result<Vec<Composition>, String> {
...
pub fn db_create_composition(
    ...
    let dilution_ids: Vec<i64> = input.ingredients.iter().map(|i| i.dilution_id).collect();
...
#[tauri::command]
pub fn db_create_formula(app: AppHandle, input: CreateFormulaInput) -> Result<Formula, String> {
    if input.composition_id <= 0 {
        return Err("Invalid composition id".into());
    }
    validate_ingredients(&input.ingredients)?;

    let total: f64 = input.ingredients.iter().map(|i| i.formula_percentage).sum();
