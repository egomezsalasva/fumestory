...
pub fn db_set_user_settings(
    ...
    let settings_raw = serde_json::to_string(&input.settings).map_err(|e| e.to_string())?;
    let dismissed_raw = serde_json::to_string(&input.dismissed_ui).map_err(|e| e.to_string())?;
